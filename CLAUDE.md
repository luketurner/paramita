...
- expo-sqlite ~16.0.10 (local SQLite database: `paramita.db`); expo-sqlite/kv-store for timer crash recovery (001-meditation-app)
- expo-sqlite ~16.0.10 (local SQLite database: `paramita.db`) — no schema changes needed (002-goal-progress-tracking)
...
- expo-sqlite ~16.0.10 (paramita.db — no schema changes); expo-sqlite/kv-store (extended persisted timer state) (003-meditation-countdown-notify)
